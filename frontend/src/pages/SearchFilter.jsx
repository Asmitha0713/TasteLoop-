    ...
    <div className="page"><CustomerNav /><main className="page-content">
      ...
      <section className="container search-layout" style={styles.layout}>
        ...
        <aside className="card" style={styles.filters}>
          ...
          <div style={styles.group}><strong style={styles.label}>Maximum price</strong><input type="range" min="50" max="1500" step="50" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} style={{ width: '100%' }} /><div style={styles.range}><span>Rs 50</span><strong>Rs {maxPrice}</strong></div></div>
...
const styles = {
  ...
  search: { maxWidth: 720, display: 'flex', alignItems: 'center', gap: 12, padding: '8px 8px 8px 18px', border: '1.5px solid var(--theme-primary)', background: 'var(--theme-surface)', borderRadius: 'var(--radius-pill)', boxShadow: '0 10px 28px rgba(49, 29, 19, .32)' }, input: { flex: 1, minWidth: 0, border: 0, outline: 0, background: 'transparent' },
