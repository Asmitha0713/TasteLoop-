...
export default function OrderHistory() {
  ...
  useEffect(() => { api.get('/orders').then(({ data }) => setOrders(data.data.map(order => ({ ...order, date: new Date(order.created_at).toLocaleDateString(), cook: order.items[0]?.cook_name || 'Home Cook', items: order.items.map(item => `${item.quantity} × ${item.name}`).join(', '), image_url: order.items[0]?.image_url, emoji: order.items[0]?.emoji || '🍽️', color: order.items[0]?.color || '#f4dfb8', status: order.status[0].toUpperCase() + order.status.slice(1).replaceAll('_', ' ') })))).catch(requestError => setError(apiError(requestError))) }, [])
