    ...
    <div className="page cook-page"><CookNav />
      <main className="page-content cook-main"><div className="container">
        ...
        <section className="card manage-card">
          ...
          <div className="food-table-wrap"><table className="food-table"><thead><tr><th>Food</th><th>Price</th><th>Portions</th><th>Status</th><th className="actions-heading">Actions</th></tr></thead><tbody>
            {visible.map((food) => <tr key={food.id}><td><div className="food-cell"><span style={{ background: food.color }}>{food.image_url ? <img src={assetUrl(food.image_url)} alt="" /> : food.emoji}</span><div><strong>{food.name}</strong><small>{food.category}</small></div></div></td><td><strong>Rs. {Number(food.price).toLocaleString()}</strong></td><td>{food.portions}</td><td><button className={`status-pill ${food.status.toLowerCase().replace(' ', '-')}`} onClick={() => toggle(food.id)}><i />{food.status}</button></td><td><div className="row-actions"><Link className="food-action-edit" to={`/cook/foods/${food.id}/edit`} aria-label={`Edit ${food.name}`}><span aria-hidden="true">✎</span> Edit</Link><button className="food-action-delete" onClick={() => remove(food.id, food.name)} aria-label={`Delete ${food.name}`}><span aria-hidden="true">🗑</span> Delete</button></div></td></tr>)}
          </tbody></table></div>
