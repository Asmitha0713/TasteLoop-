...
export default function AdminReports() {
  ...
  const display=report=>({...report,type:report.report_type,reporter:report.reporter_name||'TasteLoop user',against:report.against_name||'Marketplace user',date:new Date(report.created_at).toLocaleDateString(),status:report.status[0].toUpperCase()+report.status.slice(1),priority:report.priority[0].toUpperCase()+report.priority.slice(1)})
  useEffect(()=>{api.get('/admin/reports').then(({data})=>setReports(data.data.map(display))).catch(requestError=>setError(apiError(requestError)))},[])
  const update=async(id,status)=>{try{await api.patch(`/admin/reports/${id}`,{status:status.toLowerCase()});setReports(current=>current.map(r=>r.id===id?{...r,status}:r));setSelected(current=>current?.id===id?{...current,status}:current)}catch(requestError){setError(apiError(requestError))}}
  const resolve=id=>{update(id,'Resolved');setSelected(null)}
