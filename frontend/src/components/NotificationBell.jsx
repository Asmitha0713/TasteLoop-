...
export default function NotificationBell() {
  ...
  const load = () => api.get('/notifications', { params: { limit: 20 }, skipToast: true })
    .then(({ data }) => { setItems(data.data); setUnread(data.unread_count) })
    .catch(() => {})
  ...
  return <div className="notification-wrap" ref={wrap}>
    ...
    {open && <section className="notification-panel">
      ...
      <div className="notification-list">{items.length ? items.map(item => <button type="button" key={item.id} className={item.read ? '' : 'unread'} onClick={() => markRead(item)}><i>{item.type === 'new_order' ? '🍽️' : item.type === 'order_delivered' ? '✓' : '▣'}</i><span><strong>{item.title}</strong><small>{item.message}</small><time>{new Date(item.created_at).toLocaleString()}</time></span></button>) : <div className="notification-empty">No notifications yet.</div>}</div>
    </section>}
  </div>
}
